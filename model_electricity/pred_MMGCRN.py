...
parser.add_argument('--seq_len', type=int, default=8, help='sequence length of values')
parser.add_argument('--his_len', type=int, default=12, help='sequence length of observed historical values')
parser.add_argument('--city', type=str, default='ELECTRICITY2014', help='which experiment setting (city) to run')
...
if opt.history: feature_list.append('consumption_y')

_, filename = os.path.split(os.path.abspath(sys.argv[0]))
filename = os.path.splitext(filename)[0]
model_name = opt.model
timestring = time.strftime('%Y%m%d%H%M%S', time.localtime())
path = f'../save/{opt.city}_{model_name}_c{opt.channelin}to{opt.channelout}_{timestring}_{opt.ycov}' 
...
def main():
    data_path = '../ELECTRICITY/electricity2014.csv.gz'
